...
#ifndef next_block_index
#define next_block_index(x) BLOCK_MOD(x+1)
#define prev_block_index(x) BLOCK_MOD(x-1)
#endif
...
class Planner {
  ...
  private:
     ...
     */
    //static constexpr uint8_t IRAM_ATTR next_block_index(const uint8_t block_index) { return BLOCK_MOD(block_index + 1); }
    //static constexpr uint8_t IRAM_ATTR prev_block_index(const uint8_t block_index) { return BLOCK_MOD(block_index - 1); }
