...
void InterfaceSoundsScreen::onRedraw(draw_mode_t what) {
  ...
  if (what & BACKGROUND) {
    ...
    font(font_medium);
    text(BTN_POS(1,1), BTN_SIZE(4,1), (const char*)GET_TEXT_F(MSG_SOUNDS));
    #undef EDGE_R
    #define EDGE_R 30
    font(font_small);
    tag(0);
    text(BTN_POS(1,2), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_SOUND_VOLUME),   OPT_RIGHTX | OPT_CENTERY);
    text(BTN_POS(1,3), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_CLICK_SOUNDS),   OPT_RIGHTX | OPT_CENTERY);
    text(BTN_POS(1,5), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_PRINT_STARTING), OPT_RIGHTX | OPT_CENTERY);
    text(BTN_POS(1,6), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_PRINT_FINISHED), OPT_RIGHTX | OPT_CENTERY);
    text(BTN_POS(1,7), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_PRINT_ERROR),    OPT_RIGHTX | OPT_CENTERY);
    #undef EDGE_R
  }
  ...
  if (what & FOREGROUND) {
    ...
    colors(ui_toggle);
    tag(3);toggle2   (BTN_POS(3,3), BTN_SIZE(w,1), (const char*)GET_TEXT_F(MSG_NO), (const char*)GET_TEXT_F(MSG_YES), UIData::touch_sounds_enabled());
    #undef EDGE_R
    colors(normal_btn);
    #define EDGE_R 0
    ...
    colors(action_btn);
    tag(1);button(BTN_POS(1,9), BTN_SIZE(4,1), (const char*)GET_TEXT_F(MSG_BACK));
  }
}
...
bool InterfaceSoundsScreen::onTouchStart(uint8_t tag) {
  ...
  #define EDGE_R 30
  switch (tag) {
    case 2: track_linear(BTN_POS(3,2), BTN_SIZE(2,1), 2);cmd_execute(); break;
    default: break;
  }
  return true;
}
