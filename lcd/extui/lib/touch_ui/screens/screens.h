...
class BaseScreen : public UIScreen {
  ...
  public:
    static bool buttonStyleCallback(uint8_t, uint8_t &, uint16_t &, bool);
...
class StatusScreen : public BaseScreen, public CachedScreen<STATUS_SCREEN_CACHE,STATUS_SCREEN_DL_SIZE> {
  private:
    static void draw_axis_position();
    static void draw_temperature();
    static void draw_progress();
    static void draw_interaction_buttons();
  ...
  protected:
    class widgets_t {
      ...
      protected:
        ...
        void _button(uint8_t tag, int16_t x, int16_t y, int16_t w, int16_t h, progmem_str, bool enabled = true, bool highlight = false);
        void _button_style(style_t style);
      public:
...
class InterfaceSoundsScreen : public BaseScreen, public CachedScreen<INTERFACE_SOUNDS_SCREEN_CACHE> {
  ...
  private:
    ...
    static void setSoundSelection(event_t, const ExtUI::SoundPlayer::sound_t*);

  public:
    static void playEventSound(event_t, ExtUI::play_mode_t = ExtUI::PLAY_ASYNCHRONOUS);
