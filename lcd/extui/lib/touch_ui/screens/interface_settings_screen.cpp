...
void InterfaceSettingsScreen::onEntry() {
  screen_data.InterfaceSettingsScreen.brightness = get_brightness();
...
void InterfaceSettingsScreen::onRedraw(draw_mode_t what) {
  if (what & BACKGROUND) {
    ...
    tag(0);
    font(font_medium);
    text(BTN_POS(1,1), BTN_SIZE(4,1), (const char*)GET_TEXT_F(MSG_INTERFACE));

    #undef EDGE_R
    #define EDGE_R 30
    font(font_small);
    tag(0);
    text(BTN_POS(1,2), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_LCD_BRIGHTNESS), OPT_RIGHTX | OPT_CENTERY);
    text(BTN_POS(1,3), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_SOUND_VOLUME),   OPT_RIGHTX | OPT_CENTERY);
    text(BTN_POS(1,4), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_SCREEN_LOCK),    OPT_RIGHTX | OPT_CENTERY);
    #if DISABLED(TOUCH_UI_NO_BOOTSCREEN)
    text(BTN_POS(1,5), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_BOOT_SCREEN),    OPT_RIGHTX | OPT_CENTERY);
    #endif
    #undef EDGE_R
  }
  ...
  if (what & FOREGROUND) {
    ...
    #define EDGE_R 30
       ...
       colors(ui_toggle);
       tag(4);toggle2(BTN_POS(3,4), BTN_SIZE(w,1), (const char*)GET_TEXT_F(MSG_NO), (const char*)GET_TEXT_F(MSG_YES), LockScreen::is_enabled());
    #if DISABLED(TOUCH_UI_NO_BOOTSCREEN)
       tag(5);toggle2(BTN_POS(3,5), BTN_SIZE(w,1), (const char*)GET_TEXT_F(MSG_NO), (const char*)GET_TEXT_F(MSG_YES), UIData::animations_enabled());
    #endif
    #undef EDGE_R
    #define EDGE_R 0
    #ifdef TOUCH_UI_PORTRAIT
       colors(normal_btn);
       tag(6);button (BTN_POS(1,6), BTN_SIZE(4,1), (const char*)GET_TEXT_F(MSG_SOUNDS));
       colors(action_btn);
       tag(1);button (BTN_POS(1,7), BTN_SIZE(4,1), (const char*)GET_TEXT_F(MSG_BACK));
    #else
       tag(6);button (BTN_POS(1,6), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_SOUNDS));
       colors(action_btn);
       tag(1);button (BTN_POS(3,6), BTN_SIZE(2,1), (const char*)GET_TEXT_F(MSG_BACK));
    #endif
  }
}
...
bool InterfaceSettingsScreen::onTouchStart(uint8_t tag) {
  ...
  #define EDGE_R 30
  switch (tag) {
    case 2: track_linear(BTN_POS(3,3), BTN_SIZE(2,1), 2);cmd_execute(); break;
    case 3: track_linear(BTN_POS(3,4), BTN_SIZE(2,1), 3);cmd_execute(); break;
    default: break;
  }
  #undef EDGE_R
  #define EDGE_R 0
...
void InterfaceSettingsScreen::onIdle() {
  if (refresh_timer.elapsed(TOUCH_UPDATE_INTERVAL)) {
    ...
    uint16_t value;
    switch (track_tag(value)) {
      case 2:
        screen_data.InterfaceSettingsScreen.brightness = float(value) * 128 / 0xFFFF;
        set_brightness(screen_data.InterfaceSettingsScreen.brightness);
        SaveSettingsDialogBox::settingsChanged();
        break;
      case 3:
...
void InterfaceSettingsScreen::saveSettings(char *buff) {
  ...
  eeprom.display_brightness   = get_brightness();
  eeprom.bit_flags            = UIData::get_persistent_data();
  eeprom.touch_transform_a    = 0;//CLCD::mem_read_32(CLCD::REG::TOUCH_TRANSFORM_A);
  eeprom.touch_transform_b    = 0;//CLCD::mem_read_32(CLCD::REG::TOUCH_TRANSFORM_B);
  eeprom.touch_transform_c    = 0;//CLCD::mem_read_32(CLCD::REG::TOUCH_TRANSFORM_C);
  eeprom.touch_transform_d    = 0;//CLCD::mem_read_32(CLCD::REG::TOUCH_TRANSFORM_D);
  eeprom.touch_transform_e    = 0;//CLCD::mem_read_32(CLCD::REG::TOUCH_TRANSFORM_E);
  eeprom.touch_transform_f    = 0;//CLCD::mem_read_32(CLCD::REG::TOUCH_TRANSFORM_F);
...
void InterfaceSettingsScreen::loadSettings(const char *buff) {
  ...
  set_brightness(eeprom.display_brightness);
