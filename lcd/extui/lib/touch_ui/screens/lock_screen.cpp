...
void LockScreen::onRedraw(draw_mode_t what) {
  ...
  if (what & FOREGROUND) {
    ...
    font(font_large);
    #ifdef TOUCH_UI_PORTRAIT
       text(BTN_POS(1,2), BTN_SIZE(1,1), (const char*)message);
       font(font_xlarge);
       text(BTN_POS(1,4), BTN_SIZE(1,1), (const char*)screen_data.LockScreen.passcode);
    #else
       text(BTN_POS(1,1), BTN_SIZE(1,1), (const char*)message);
       font(font_xlarge);
       text(BTN_POS(1,2), BTN_SIZE(1,1), (const char*)screen_data.LockScreen.passcode);
    #endif
       font(font_large);
       colors(normal_btn);
    #ifdef TOUCH_UI_PASSCODE
       keys(BTN_POS(1,l+1), BTN_SIZE(1,1), (const char*)F("123"),        pressed);
       keys(BTN_POS(1,l+2), BTN_SIZE(1,1), (const char*)F("456"),        pressed);
       keys(BTN_POS(1,l+3), BTN_SIZE(1,1), (const char*)F("789"),        pressed);
       keys(BTN_POS(1,l+4), BTN_SIZE(1,1), (const char*)F("0.<"),        pressed);
    #else
       keys(BTN_POS(1,l+1), BTN_SIZE(1,1), (const char*)F("1234567890"), pressed);
       keys(BTN_POS(1,l+2), BTN_SIZE(1,1), (const char*)F("qwertyuiop"), pressed);
       keys(BTN_POS(1,l+3), BTN_SIZE(1,1), (const char*)F("asdfghjkl "), pressed);
       keys(BTN_POS(1,l+4), BTN_SIZE(1,1), (const char*)F("zxcvbnm!?<"), pressed);
    #endif
