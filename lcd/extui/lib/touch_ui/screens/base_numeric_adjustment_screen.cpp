 ...
 */
void BaseNumericAdjustmentScreen::widgets_t::_button_style(BaseNumericAdjustmentScreen::widgets_t::style_t style) {
  if (_style != style) {
    ...
    if (rgb_changed)  color_rgb(style == TEXT_LABEL ? bg_text_enabled : new_colors->rgb);
    if (grad_changed) gradcolor(new_colors->grad);
    if (fg_changed)   fgcolor(new_colors->fg);
    if (bg_changed)   bgcolor(new_colors->bg);
 ...
 */
void BaseNumericAdjustmentScreen::widgets_t::_button(uint8_t tag, int16_t x, int16_t y, int16_t w, int16_t h, progmem_str text, bool enabled, bool highlight) {
  if (_what & BACKGROUND) enabled = true;
  if ((_what & BACKGROUND) || buttonIsPressed(tag) || highlight || !enabled) {
    _button_style((!enabled) ? BTN_DISABLED : (highlight ? BTN_ACTION : BTN_NORMAL));
    ExtUI::tag(enabled ? tag : 0);ExtUI::button(x, y, w, h, (const char*)text);
  }
}
...
void BaseNumericAdjustmentScreen::widgets_t::_draw_increment_btn(uint8_t, const uint8_t tag) {
  ...
  switch (pos) {
    #ifdef TOUCH_UI_PORTRAIT
      case 0: _button(tag, BTN_POS(5,_line), BTN_SIZE(2,1), progmem_str(label), true, highlight); break;
      case 1: _button(tag, BTN_POS(7,_line), BTN_SIZE(2,1), progmem_str(label), true, highlight); break;
      case 2: _button(tag, BTN_POS(9,_line), BTN_SIZE(2,1), progmem_str(label), true, highlight); break;
    #else
      case 0: _button(tag, BTN_POS(15,2),    BTN_SIZE(4,1), progmem_str(label), true, highlight); break;
      case 1: _button(tag, BTN_POS(15,3),    BTN_SIZE(4,1), progmem_str(label), true, highlight); break;
      case 2: _button(tag, BTN_POS(15,4),    BTN_SIZE(4,1), progmem_str(label), true, highlight); break;
    #endif
  }
}
...
void BaseNumericAdjustmentScreen::widgets_t::increments() {
  ...
  if (_what & BACKGROUND) {
    ...
    tag(0);text(
      ...
      #endif
      (const char*)GET_TEXT_F(MSG_INCREMENT)
    );
  }
...
void BaseNumericAdjustmentScreen::widgets_t::adjuster_sram_val(uint8_t tag, progmem_str label, const char *value, bool is_enabled) {
  ...
  if (_what & BACKGROUND) {
    ...
    ExtUI::tag(0);
    font(font_small);
    text( BTN_POS(1,_line), BTN_SIZE(4,1), (const char*)label);
    _button_style(TEXT_AREA);
    fgcolor(_color);ExtUI::button( BTN_POS(5,_line), BTN_SIZE(5,1), (const char*)F(""), OPT_FLAT);
  }

  font(font_medium);
  _button(tag,     BTN_POS(10,_line), BTN_SIZE(2,1),  F("-"), is_enabled);
  _button(tag + 1, BTN_POS(12,_line), BTN_SIZE(2,1),  F("+"), is_enabled);

  if ((_what & FOREGROUND) && is_enabled) {
    _button_style(BTN_NORMAL);
    ExtUI::tag(0);
    font(font_small);
...
void BaseNumericAdjustmentScreen::widgets_t::button(uint8_t tag, progmem_str label, bool is_enabled) {
  font(LAYOUT_FONT);
  _button(tag, BTN_POS(5,_line), BTN_SIZE(9,1), label, is_enabled);
...
void BaseNumericAdjustmentScreen::widgets_t::text_field(uint8_t tag, progmem_str label, const char *value, bool is_enabled) {
  ...
  if (_what & BACKGROUND) {
    ...
    fgcolor(_color);
    ExtUI::tag(tag);
    ExtUI::button( BTN_POS(5,_line), BTN_SIZE(9,1), (const char*)F(""), OPT_FLAT);
  }

  if (_what & FOREGROUND) {
    font(font_small);text( BTN_POS(5,_line), BTN_SIZE(9,1), is_enabled ? value : "-");
  }
...
void BaseNumericAdjustmentScreen::widgets_t::two_buttons(uint8_t tag1, progmem_str label1, uint8_t tag2, progmem_str label2, bool is_enabled) {
  font(LAYOUT_FONT);
  _button(tag1, BTN_POS(5,_line),   BTN_SIZE(4.5,1), label1, is_enabled);
  _button(tag2, BTN_POS(9.5,_line), BTN_SIZE(4.5,1), label2, is_enabled);
...
void BaseNumericAdjustmentScreen::widgets_t::toggle(uint8_t tag, progmem_str label, bool value, bool is_enabled) {
  ...
  if (_what & FOREGROUND) {
    _button_style(BTN_TOGGLE);
    ExtUI::tag(is_enabled ? tag   : 0);
    enabled(is_enabled);
    font(font_small);
    toggle2(
        #ifdef TOUCH_UI_PORTRAIT
        ...
        #endif
        (const char*)GET_TEXT_F(MSG_NO), (const char*)GET_TEXT_F(MSG_YES), value
       );
  }
...
void BaseNumericAdjustmentScreen::widgets_t::home_buttons(uint8_t tag) {
  ...
  if (_what & BACKGROUND) {
    _button_style(TEXT_LABEL);
    font(font_small);
    text(BTN_POS(1, _line), BTN_SIZE(4,1), (const char*)GET_TEXT_F(MSG_HOME));
  }

  font(LAYOUT_FONT);
 _button(tag+0, BTN_POS(5,_line),    BTN_SIZE(2,1), GET_TEXT_F(MSG_AXIS_X));
 _button(tag+1, BTN_POS(7,_line),    BTN_SIZE(2,1), GET_TEXT_F(MSG_AXIS_Y));
 #if DISABLED(Z_SAFE_HOMING)
   _button(tag+2, BTN_POS(9,_line),  BTN_SIZE(2,1), GET_TEXT_F(MSG_AXIS_Z));
   _button(tag+3, BTN_POS(11,_line), BTN_SIZE(3,1), GET_TEXT_F(MSG_AXIS_ALL));
 #else
   _button(tag+3, BTN_POS(9,_line),  BTN_SIZE(3,1), GET_TEXT_F(MSG_AXIS_ALL));
 #endif
...
void BaseNumericAdjustmentScreen::onEntry() {
  ...
  BaseScreen::onEntry();
  set_button_style_callback(nullptr);
}
