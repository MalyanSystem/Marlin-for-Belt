...
void AdvancedSettingsMenu::onRedraw(draw_mode_t what) {
  ...
  if (what & FOREGROUND) {
   ...
   tag(2);button( ZPROBE_ZOFFSET_POS,     (const char*)GET_TEXT_F(MSG_ZPROBE_ZOFFSET));
   enabled(
        #if HAS_CASE_LIGHT
          1
        #endif
      );
   tag(16);button( CASE_LIGHT_POS,         (const char*)GET_TEXT_F(MSG_CASE_LIGHT));
   tag(3);button( STEPS_PER_MM_POS,       (const char*)GET_TEXT_F(MSG_STEPS_PER_MM));
   enabled(
        #if HAS_TRINAMIC_CONFIG
          1
        #endif
      );
   tag(13);button( TMC_CURRENT_POS,        (const char*)GET_TEXT_F(MSG_TMC_CURRENT));
   enabled(
        #if ENABLED(SENSORLESS_HOMING)
          1
        #endif
      );
   tag(14);button( TMC_HOMING_THRS_POS,    (const char*)GET_TEXT_F(MSG_TMC_HOMING_THRS));
   enabled(
        #if HOTENDS > 1
          1
        #endif
      );
   tag(4);button( OFFSETS_POS,            (const char*)GET_TEXT_F(MSG_OFFSETS_MENU));
   enabled(
        #if EITHER(LIN_ADVANCE, FILAMENT_RUNOUT_SENSOR)
          1
        #endif
      );
   tag(11);button( FILAMENT_POS,           (const char*)GET_TEXT_F(MSG_FILAMENT));
   tag(12);button( ENDSTOPS_POS,           (const char*)GET_TEXT_F(MSG_LCD_ENDSTOPS));
   tag(15);button( DISPLAY_POS,            (const char*)GET_TEXT_F(MSG_DISPLAY_MENU));
   tag(9);button( INTERFACE_POS,          (const char*)GET_TEXT_F(MSG_INTERFACE));
   tag(10);button( RESTORE_DEFAULTS_POS,   (const char*)GET_TEXT_F(MSG_RESTORE_DEFAULTS));
   tag(5);button( VELOCITY_POS,           (const char*)GET_TEXT_F(MSG_VELOCITY));
   tag(6);button( ACCELERATION_POS,       (const char*)GET_TEXT_F(MSG_ACCELERATION));
   tag(7);button( JERK_POS,               (const char*)GET_TEXT_F(
   ...
   tag(8);button( BACKLASH_POS,            (const char*)GET_TEXT_F(MSG_BACKLASH));
   colors(action_btn);
