...
void EventLoop::process_events() {
  ...
  if (!touch_timer.elapsed(TOUCH_UPDATE_INTERVAL)) {
    return;
  }

  const uint8_t tag = ExtUI::get_tag();
    ...
    case UNPRESSED:
      if (tag != 0) {
        ...
        if (current_screen.onTouchStart(tag)) {
          touch_timer.start();
          if (UIData::flags.bits.touch_start_sound) ExtUI::sound.play(press_sound);
        }
    ...
    default: // PRESSED
      if (!UIData::flags.bits.touch_debouncing) {
        if (tag == pressed_tag) {
          ...
          if (touch_timer.elapsed(1000 / TOUCH_REPEATS_PER_SECOND)) {
            if (current_screen.onTouchHeld(tag)) {
              current_screen.onRefresh();
              if (UIData::flags.bits.touch_repeat_sound) ExtUI::sound.play(repeat_sound);
            }
            touch_timer.start();
          }
        }
        ...
        if (tag == pressed_tag) {
          ...
          if (UIData::flags.bits.touch_end_sound) ExtUI::sound.play(unpress_sound);
