...
namespace ExtUI {
   ...
   */
  void draw_text_box(int x, int y, int w, int h, const char *str, uint16_t options, uint8_t font) {
    ...
    for(;;) {
      ...
      if (line_len) {
        ...
        #endif
          {
            text(x + dx, y + dy, font, options & ~OPT_CENTERY);
            ExtUI::str(line);
          }
      }
      y += fm.get_height();
  ...
  void draw_text_box(int x, int y, int w, int h, progmem_str pstr, uint16_t options, uint8_t font) {
    char str[strlen((const char*)pstr) + 1];
    strcpy(str, (const char*)pstr);
    draw_text_box(x, y, w, h, (const char*) str, options, font);
  }
} // namespace FTDI
