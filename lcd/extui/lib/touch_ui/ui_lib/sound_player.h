...
namespace ExtUI {
  ...
  class SoundPlayer {
    public:
      struct sound_t {
        ...
        ui_note_t    note;        // The MIDI note value
    ...
    private:
      ...
      ui_note_t frequency_to_midi_note(const uint16_t frequency);

    public:
      static void set_volume(uint8_t volume);
      static uint8_t get_volume();

      static void play(effect_t effect, ui_note_t note = NOTE_C4);
  ...
  const SoundPlayer::sound_t silence[] = {
    {SILENCE,      END_SONG, 0}
  };
}
