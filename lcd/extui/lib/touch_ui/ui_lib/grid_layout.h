...
// the layout grid.
#define GRID_X(x)        ((x)*(ExtUI::display_width-EDGE_R)/GRID_COLS)
#define GRID_Y(y)        ((y)*ExtUI::display_height/GRID_ROWS)
...
#define DRAW_LAYOUT_GRID \
  { \
    ...
    for(int i = 1; i <= GRID_COLS; i++) { \
      ...
      cmd.cmd(VERTEX2F(GRID_X(i) *16, ExtUI::display_height *16)); \
    } \
    for(int i = 1; i < GRID_ROWS; i++) { \
      cmd.cmd(BEGIN(LINES)); \
      cmd.cmd(VERTEX2F(0                       *16, GRID_Y(i) *16)); \
      cmd.cmd(VERTEX2F(ExtUI::display_width     *16, GRID_Y(i) *16)); \
    } \
