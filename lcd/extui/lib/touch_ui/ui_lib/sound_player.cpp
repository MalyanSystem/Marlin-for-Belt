...
namespace ExtUI {
  ...
  uint8_t SoundPlayer::get_volume() {
    return 0;//CLCD::mem_read_8(REG::VOL_SOUND);
  }

  void SoundPlayer::play(effect_t effect, ui_note_t note) {
    printf("Playing note %d, instrument %d\n",note,effect);
  }

  ui_note_t SoundPlayer::frequency_to_midi_note(const uint16_t frequency_hz) {
    const float f0 = 440;
    return ui_note_t(NOTE_A4 + (log(frequency_hz)-log(f0))*12/log(2) + 0.5);
  }
  ...
  bool SoundPlayer::is_sound_playing() {
    return 0;//CLCD::mem_read_8( REG::PLAY ) & 0x1;
  }
  ...
  void SoundPlayer::onIdle() {
    ...
    if (ready_for_next_note) {
      const effect_t fx = effect_t(sequence->effect);
      const ui_note_t   nt =   ui_note_t(sequence->note);
      const uint32_t ms = uint32_t(sequence->sixteenths) * 1000 / 16;
